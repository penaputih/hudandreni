...
// rsvp
const scriptUrl = "https://script.google.com/macros/s/AKfycbzSnLLcXAA9JPndHa8xRia8xxiims4KTinIXlKb7_xckkmYLUMGxaj7p_SgrU8PpkdQ/exec";

const form = document.getElementById("rsvp-form");
const rsvpList = document.getElementById("rsvp-list");
const modal = document.getElementById("rsvpModal");
const btnRsvp = document.getElementsByClassName("btn-rsvp");
...
function addRsvpCard({ nama, name, kehadiran, attendance, ucapan, message, timestamp }) {
  const guestName = nama || name;
  const hadir = kehadiran || attendance;
  const pesan = ucapan || message;

  const div = document.createElement("div");
  div.className = "rsvp-item";
  div.innerHTML = `
    <div class="avatar">${getInitials(guestName).toUpperCase()}</div>
    <div class="rsvp-content">
      <div class="rsvp-name">${guestName} <span class="badge">${hadir}</span></div>
      <div class="rsvp-message">${pesan || ""}</div>
      <div class="rsvp-time">${formatTime(new Date(timestamp))}</div>
    </div>`;
  rsvpList.prepend(div); // tampilkan RSVP terbaru di atas
}

function renderRSVPs(data) {
  rsvpList.innerHTML = "";
  data.forEach(addRsvpCard);
}
...
for (let i = 0; i < btnRsvp.length; i++) {
  btnRsvp[i].onclick = function () {
    ...
    // Kalau cachedRSVPs belum siap, fetch dulu
    if (cachedRSVPs.length === 0) {
      fetchRSVPs(); // fetch + render
    } else {
      renderRSVPs(cachedRSVPs); // tampil langsung
    }
  };
}

function renderRSVPs(data) {
  rsvpList.innerHTML = data.length === 0
    ? "<p style='text-align:center; color:#999;'>Memuat Konten</p>"
    : "";
